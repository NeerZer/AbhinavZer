...
const CONFIG = {
  ...
  maxHtmlContentLength: parseInt(
    process.env.MAX_HTML_CONTENT_LENGTH || "4000",
    10
  ),
  wpApiUrl:
  ...
  groqRateLimit: {
    ...
    retryAfterMultiplier: parseFloat(
      process.env.GROQ_RETRY_AFTER_MULTIPLIER || "1.5"
    ),
  },
...
if (
  !CONFIG.googleSheet.serviceAccount.email ||
  !CONFIG.googleSheet.serviceAccount.privateKey
) {
  throw new Error(
    "Google Sheet service account credentials (GOOGLE_SERVICE_ACCOUNT_EMAIL, GOOGLE_PRIVATE_KEY) are required in your .env file."
  );
}
if (!CONFIG.wpUser || !CONFIG.wpPass) {
  console.warn(
    "WARNING: WordPress credentials (WP_USER, WP_PASS) are not fully configured. WordPress API calls might fail."
  );
}
...
class EnhancedRateLimiter {
  constructor(maxRequests, intervalMs) {
    ...
    this.pausedUntil = 0;
    console.log(
      `RateLimiter: Configured for ${maxRequests} requests per ${
        intervalMs / 1000
      }s`
    );
  }
  ...
  async waitForAvailability() {
    ...
    console.log(
      `RateLimiter: Rate limit hit (${this.requestTimestamps.length}/${
        this.maxRequests
      } requests in ${this.intervalMs / 1000}s). Waiting ${timeToWait}ms...`
    );
  ...
  async executeRequest(requestFn) {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response?.status === 429) {
        ...
        console.warn(
          `RateLimiter: 429 received from API. Pausing for ${retryAfter}ms as instructed.`
        );
  ...
  calculateRetryAfter(error) {
    ...
    if (retryAfterHeader) {
      ...
      let calculatedDelay =
        seconds * 1000 * CONFIG.groqRateLimit.retryAfterMultiplier;
    ...
    return Math.min(
      CONFIG.maxBackoff,
      CONFIG.backoffBase *
        Math.pow(2, this.requestTimestamps.length - this.maxRequests)
    );
  }
}
...
class NewsProcessor {
  ...
  async initialize() {
    ...
    console.log(
      `NewsProcessor initialized with ${this.validNSC.size} symbols from Google Sheet.`
    );
  }
  ...
  async loadSymbolsFromSheet() {
    ...
    while (sheetLoadAttempts < CONFIG.maxRetries) {
      try {
        console.log(
          `NewsProcessor: Loading symbols from Google Sheet (attempt ${
            sheetLoadAttempts + 1
          })...`
        );
        ...
        const symbolCol = headers.findIndex(
          (header) => header.toLowerCase() === "symbol"
        );
        if (symbolCol === -1) {
        ...
        console.log(
          `NewsProcessor: Successfully loaded ${this.validNSC.size} symbols.`
        );
        return;
      } catch (err) {
        sheetLoadAttempts++;
        console.error(
          `NewsProcessor: Failed to load symbols (attempt ${sheetLoadAttempts}/${CONFIG.maxRetries}):`,
          err.message
        );

        if (sheetLoadAttempts >= CONFIG.maxRetries) {
          throw new Error(
            `NewsProcessor: Max retry attempts reached for loading symbols. Last error: ${err.message}`
          );
        }
        ...
        console.log(
          `NewsProcessor: Retrying symbol load in ${delay / 1000} seconds...`
        );
        await sleep(delay);
      }
    }
  ...
  async addToQueue(items) {
    ...
    console.log(
      `NewsProcessor: Received ${items.length} potential items to add to queue.`
    );
    ...
    const newItems = items
      ...
      .filter(
        (item) =>
          !this.queue.some(
            (existing) => existing.htmlContent === item.htmlContent
          )
      )
      .map((item) => ({
        ...item,

        htmlContent: this.preprocessHtml(
          item.htmlContent.substring(0, CONFIG.maxHtmlContentLength)
        ),
      }));
    ...
    if (newItems.length > 0) {
      ...
      console.log(
        `NewsProcessor: Added ${newItems.length} new items to queue. Total queue size: ${this.queue.length}.`
      );
      ...
      if (!this.isProcessing) {
        ...
        this.processQueue().catch((err) => {
          console.error(
            "NewsProcessor: Critical error during queue processing:",
            err
          );
        });
      }
      ...
      console.log(
        "NewsProcessor: No new items to add to queue or all are duplicates."
      );
    }
  }
  ...
  preprocessHtml(html) {
    try {
      ...
      const source =
        doc.querySelector(".feed")?.textContent?.replace("—", "").trim() || "";
  ...
  async processQueue() {
    ...
    try {
      while (this.queue.length > 0) {
        ...
        console.log(
          `NewsProcessor: Processing item from queue. ${this.queue.length} items remaining.`
        );
        ...
        try {
          ...
          console.error(
            "NewsProcessor: Error processing single item:",
            err.message
          );
          ...
          if (this.queue.length < 500) {
            ...
            console.warn(
              "NewsProcessor: Not requeuing item due to large queue size or persistent errors."
            );
          }
        }
      }
      ...
      console.error(
        "NewsProcessor: Unhandled error during queue processing loop:",
        queueError
      );
    } finally {
      this.isProcessing = false;
      ...
      if (
        typeof this.onQueueComplete === "function" &&
        this.queue.length === 0
      ) {
  ...
  async processItem(item) {
    ...
    const processedResult = {
      company_name: this.cleanseText(
        aiResult.company_name || "Unknown Company"
      ),
      ...
      source: this.cleanseText(
        aiResult.source?.replace("—", "").trim() || "Unknown"
      ),
      url: aiResult.url?.trim() || null,
    };

    console.log(
      `NewsProcessor: AI analysis returned NSC: '${processedResult.nsc}'`
    );

    if (processedResult.nsc && this.validNSC.has(processedResult.nsc)) {
      console.log(
        `NewsProcessor: Valid NSC found: ${processedResult.nsc}. Storing to primary endpoint.`
      );
      await this.storeResult({
        title: processedResult.headline,
        description: processedResult.description,
        source: processedResult.source,
        company: processedResult.company_name,
        nsc: processedResult.nsc,
        confidence: 0,
        news_date: processedResult.news_date,
      });
    } else {
      console.log(
        `NewsProcessor: NSC '${processedResult.nsc}' is null or not in valid list. Storing to extra endpoint.`
      );
      await this.storeExtraResult({
        title: processedResult.headline,
        description: processedResult.description,
        source: processedResult.source,
        company: processedResult.company_name,
  ...
  async analyzeWithAI(item) {
    ...
    for (let attempt = 1; attempt <= CONFIG.maxRetries; attempt++) {
      try {
        console.log(
          `NewsProcessor: Sending AI request (attempt ${attempt}/${CONFIG.maxRetries})...`
        );
        ...
        const errorMessage = axios.isAxiosError(err)
          ...
          : err.message;
        console.error(
          `NewsProcessor: AI request failed (attempt ${attempt}/${CONFIG.maxRetries}):`,
          errorMessage
        );
        ...
        if (this.consecutiveGroqErrors >= CONFIG.maxConsecutiveErrors) {
          console.error(
            `NewsProcessor: Too many consecutive Groq errors (${this.consecutiveGroqErrors}). Pausing for 5 minutes (300000ms) before resetting counter...`
          );
          await sleep(300000);
          this.consecutiveGroqErrors = 0;
        }
        ...
        if (attempt < CONFIG.maxRetries) {
          ...
          console.log(
            `NewsProcessor: Retrying Groq API call in ${
              delay / 1000
            } seconds...`
          );
          await sleep(delay);
    ...
    console.error(
      "NewsProcessor: Max retry attempts reached for Groq AI call. Giving up on this item."
    );
    return null;
  }
  ...
  createPrompt(htmlContent, companyList) {
    ...
    const truncatedCompanyList =
      companyList.length > maxCompanyListLength
        ? companyList.substring(
            0,
            companyList.lastIndexOf(",", maxCompanyListLength)
          ) + "..."
        : companyList;
...
HTML: ${htmlContent}`;
  ...
  parseAIResponse(response) {
    try {
      ...
      if (
        ...
        console.warn(
          "NewsProcessor: AI response JSON is invalid or missing/incorrect NSC. Raw AI response:",
          content
        );
        return null;
      }
      ...
      delete result.Body;
      result.company_name = result.company_name || null;
      result.confidence = 0;
      result.news_date = new Date().toISOString().split("T")[0];
      return result;
    } catch (err) {
      console.error(
        "NewsProcessor: Failed to parse AI response JSON:",
        err.message
      );
      console.error(
        "Raw AI content that failed to parse:",
  ...
  async storeResult(data) {
    if (!CONFIG.wpApiUrl || !CONFIG.wpUser || !CONFIG.wpPass) {
      console.log(
        "NewsProcessor: Primary WordPress storage skipped (WP_API_URL or credentials not fully configured)."
      );
      return;
    }
    ...
    try {
      console.log(
        `NewsProcessor: Attempting to store primary data for NSC: ${data.nsc} to WordPress.`
      );
      ...
      if (response.data?.id) {
        console.log(
          `NewsProcessor: Successfully stored primary item ID ${
            response.data.id
          } for ${data.nsc} - "${(data.title || "").substring(0, 30)}..."`
        );
      } else {
        console.warn(
          "NewsProcessor: Primary WordPress storage responded without an ID in data:",
          response.data
        );
      }
    } catch (error) {
      console.error(
        "NewsProcessor: Primary WordPress storage failed:",
        error.response?.data?.message || error.message
      );
      if (axios.isAxiosError(error) && error.response) {
        console.error(
          "WordPress Primary API Error Response Status:",
          error.response.status
        );
        console.error(
          "WordPress Primary API Error Response Data:",
          error.response.data
        );
      }
  ...
  async storeExtraResult(data) {
    if (!CONFIG.wpExtraApiUrl || !CONFIG.wpUser || !CONFIG.wpPass) {
      console.log(
        "NewsProcessor: Extra WordPress storage skipped (WP_EXTRA_API_URL or credentials not fully configured)."
      );
      return;
    }
    ...
    try {
      console.log(
        `NewsProcessor: Attempting to store extra data (NSC null/invalid) to WordPress.`
      );

      const payload = {
        Headline: data.title || "No Headline Provided",
      ...
      if (response.data?.id) {
        console.log(
          `NewsProcessor: Successfully stored extra item ID ${
            response.data.id
          } for "${(payload.Headline || "").substring(0, 30)}..."`
        );
      } else {
        console.warn(
          "NewsProcessor: Extra WordPress storage responded without an ID in data:",
          response.data
        );
      }
    } catch (error) {
      console.error(
        "NewsProcessor: Extra WordPress storage failed:",
        error.response?.data?.message || error.message
      );
      if (axios.isAxiosError(error) && error.response) {
        console.error(
          "WordPress Extra API Error Response Status:",
          error.response.status
        );
        console.error(
          "WordPress Extra API Error Response Data:",
          error.response.data
        );
      }
...
async function scrapeNews() {
  try {
    ...
    console.log(
      'Scraper: Selector ".box.item" found. Extracting HTML content.'
    );
...
async function main() {
  console.log(
    "Main: Starting application with enhanced rate limiting and error handling..."
  );
  ...
  try {
    ...
    console.log(
      "Main: All articles processed or no new articles found. Exiting."
    );
  } catch (err) {
    console.error(
      "Main: Fatal application error during startup or main loop:",
      err
    );
